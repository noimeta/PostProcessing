...
// CoC calculation
half4 FragCoC(VaryingsSimple i) : SV_Target
{
...
// Temporal filter
half4 FragTempFilter(VaryingsSimple i) : SV_Target
{
...
half4 FragPrefilter(VaryingsSimple i) : SV_Target
{
    ...
    float2 uv0 = i.texcoord - duv.xy;
    float2 uv1 = i.texcoord - duv.zy;
    float2 uv2 = i.texcoord + duv.zy;
    float2 uv3 = i.texcoord + duv.xy;
...
half4 FragBlur(VaryingsSimple i) : SV_Target
{
    ...
    for (int si = 0; si < kSampleCount; si++)
    {
        ...
        half bgWeight = saturate((bgCoC   - dist + margin) * margin_1);
        half fgWeight = saturate((-samp.a - dist + margin) * margin_1);
    ...
    bgAcc.a = smoothstep(_MainTex_TexelSize.y, margin, samp0.a);
...
half4 Upsample(TEXTURE2D_ARGS(tex, samplerTex), float2 uv, float2 texelSize)
{
	...
	half4 acc;
	acc = SAMPLE_TEXTURE2D(tex, samplerTex, uv - duv.xy);
	acc += SAMPLE_TEXTURE2D(tex, samplerTex, uv - duv.zy);
	acc += SAMPLE_TEXTURE2D(tex, samplerTex, uv + duv.zy);
	acc += SAMPLE_TEXTURE2D(tex, samplerTex, uv + duv.xy);
	return acc / 4.0;
}
...
// Combine with source
half4 FragCombine(VaryingsSimple i) : SV_Target
{
    half4 dof = Upsample(TEXTURE2D_PARAM(_DepthOfFieldTex, sampler_DepthOfFieldTex), i.texcoord, _MainTex_TexelSize.xy);
